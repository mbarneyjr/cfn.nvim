...
function M.register(profile, stack_name)
  coroutine.wrap(function()
    ...
    if template_path_err ~= nil or not template_path then
      vim.print("cfn.nvim: cannot register template: " .. (template_path_err or "no template path"))
      return
    end
    ...
    if not profile then
      ...
      if err ~= nil or not response then
        return vim.print("cfn.nvim: error registering template: " .. (err or "invalid response from cli helper"))
      end
      ...
      if not profile then
        return vim.print("cfn.nvim: template registration cancelled: no profile chosen")
      end
    end
    local creds_err, creds = credentials.set(profile)
    if creds_err or not creds then
      return vim.print(
        "cfn.nvim: error setting credentials for profile "
          .. profile
          .. ": "
          .. (creds_err or "no credentials returned")
      )
    end
    if not stack_name then
      ...
      if err or stacks == nil then
        vim.notify("cfn.nvim: Could not list CloudFormation stacks: " .. (err or "no result"), vim.log.levels.ERROR)
        return
      end
      ...
      if picked == nil then
        return vim.print("cfn.nvim: template registration cancelled: no stack name chosen")
    ...
    vim.print("registering template: " .. profile .. "/" .. stack_name)
  end)()
end
...
function M.unregister()
  local template_path_err, template_path = buffer.get_current_buffer_template_path()
  if template_path_err ~= nil or not template_path then
    vim.print("cannot register template: " .. (template_path_err or "no template path"))
    return
  end
